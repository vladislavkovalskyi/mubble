...
def deep_link[
    ...
    *,
    ...
    if not link.startswith("tg://"):
        raise RuntimeError(
            "Invalid deep link format. The link must start with 'tg://'."
        )
...
def get_query_params(
    ...
    params = OrderedDict()
    param_names = (
        [*order_params, *(p for p in annotations if p not in order_params)]
        if order_params
        else annotations
    )
...
def parse_deep_link(
    *,
    ...
    query = (
        ...
        + ("&" if no_value_params else "")
        + "&".join(no_value_params)
    )
    return f"{link}?{query}"
...
def tg_bot_startgroup_link(
    *,
    ...
    permissions: Parameter[
        list[Permission] | None, "admin", validate_permissions
    ] = None,
) -> str:
